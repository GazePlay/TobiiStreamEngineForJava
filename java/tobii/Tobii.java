...
public class Tobii
{
    ...
    public static void execProg(TobiiGazeListener listener)
    {
        ...
        { // init was not done (no tobii available)
            log.info("Tobii detected : NO");
            return;
        }
    }

    public static String dllFolder = "dll";
    ...
    static
    {
        try
        {
            if (System.getProperty("os.name").indexOf("indow") > 0)
            {
                System.load(dllFolder + "tobii_stream_engine.dll");
                System.load(dllFolder + "jni_tobii.dll");
            }
        }
